...
impl GatedLinearAttention {
    ...
    pub fn forward(&mut self, x: Tensor3, grad: bool) -> Tensor3 {
        ...
        for t in 0..seq_len {
            ...
            let forget_expanded = forget_alpha.outer(&forget_beta);

            // (B, d_k, 1) * (B, 1, d_v) -> (B, d_k, d_v)
            let kv = k_t.outer(&v_t);
    ...
    pub fn backward(&mut self, d_loss: Tensor3) -> Tensor3 {
        ...
        for t in (0..seq_len).rev() {
            ...
            let forget_expanded_t = forget_alpha.outer(&forget_beta);
            ...
            // S' = f * S + k.dot(v.T)
            d_loss_state += q_t.outer(&d_loss_t);
