...
fn intermediate_caching() {
    let mut model = LinearTransformer::new(8, 4);
...
fn identity() {
    ...
    let mut transformer = LinearTransformer::new(
        EMBED_DIM, D_HEAD, // num_layers
    );
...
pub fn mean_pooling() {
    ...
    let mut transformer2 = LinearTransformer::new(EMBED_DIM, NUM_HEADS);
...
fn gradient_flow() {
    let mut transformer = LinearTransformer::new(
        EMBED_DIM, NUM_HEADS, // Deeper network to test gradient flow
    );
...
#[test]
fn overfitting() {
    let mut transformer = LinearTransformer::new(EMBED_DIM, NUM_HEADS);
...
fn retrieval_by_marker() {
    ...
    let mut model = LinearTransformer::new(16, 6);
...
fn fixed_position_retrieval() {
    ...
    let mut model = LinearTransformer::new(16, 8);
    let mut optim = AdamW::default().with(&mut model);
    optim.learning_rate = 1e-2;
    ...
    for e in 0..2000 {
        ...
        let y_pred = model.forward(x_pe.clone(), true);
